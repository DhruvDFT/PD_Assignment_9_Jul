...
<body>
        ...
        <div class="main-section">
            <div class="section-header">
                ...
                <div class="filter-tabs">
                    <button class="tab-btn active" onclick="filterAssignments('all')">All</button>
                    <button class="tab-btn" onclick="filterAssignments('pending')">Pending</button>
                    <button class="tab-btn" onclick="filterAssignments('submitted')">Submitted</button>
                    <button class="tab-btn" onclick="filterAssignments('completed')">Completed</button>
                </div>
            </div>
    ...
    <script>
        ...
        // Auto-refresh assignments every 2 minutes
        setInterval(() => {
            fetch('/student/refresh-assignments')
                .then(response => response.json())
                .then(data => {
                    if (data.newAssignments) {
                        location.reload();
                    }
                })
                .catch(() => {
                    // Silently fail - no need to show error for background refresh
                });
        }, 120000);
    </script>
</body>
...
def student_test(assignment_id):
    ...
    if existing_submission:
        conn.close()
        return redirect('/student')  # Already submitted
...
<body>
    ...
    <div class="navigation-buttons">
        <button class="nav-btn" id="prevBtn" onclick="navigateQuestion(-1)" disabled>←</button>
        <button class="nav-btn" id="nextBtn" onclick="navigateQuestion(1)">→</button>
    </div>
    ...
    <script>
        ...
        function showQuestion(index) {
            ...
            // Focus on textarea
            const currentTextarea = questions[index].querySelector('textarea');
            if (currentTextarea) {
        ...
        function loadAutoSavedData() {
            ...
            if (saved) {
                try {
                    ...
                    if (savedQuestion && savedQuestion < totalQuestions) {
...
@app.route('/student/refresh-assignments')
def student_refresh():
    if not session.get('user_id') or session.get('is_admin'):
        return jsonify({'error': 'Unauthorized'}), 403
    
    # Simple endpoint to check for new assignments
    return jsonify({'newAssignments': False})
...
if __name__ == '__main__':
    ...
    <div class="container">
        <div class="test-# Continue with missing routes...

@app.route('/admin/review/<assignment_id>')
def admin_review(assignment_id):
    ...
    for i, question in enumerate(questions):
        ...
        ai_feedback = auto_score_data.get('feedback', 'No AI analysis available')
        ...
        color = "#10b981" if suggested_score >= 7 else "#f59e0b" if suggested_score >= 5 else "#ef4444"
...
@app.route('/admin/review/<assignment_id>', methods=['POST'])
def admin_review_post(assignment_id):
    return admin_review(assignment_id)
...
def admin_analytics():
    ...
    c.execute('''
        SELECT 
            DATE(s.submitted_date) as date,
            COUNT(*) as submissions,
            AVG(CAST(s.total_score as FLOAT)) as avg_score
        FROM submissions s
        WHERE s.status = 'graded' AND s.submitted_date >= date('now', '-30 days')
        GROUP BY DATE(s.submitted_date)
        ORDER BY date DESC
    ''')
    analytics_data['daily_stats'] = c.fetchall()
...
<body>
    ...
    <script>
        ...
        const topicCounts = topicData.map(item => item[1] || 0);
...
def student():
    ...
    for assignment in assignments:
        status = assignment[11] or 'pending'  # submission_status
        score = assignment[12] or 0  # total_score
